...
class Circle:
    ...
    def line_in_circle(self,line):
        ...
        if self.contain_point(line.x, line.y) and self.line_intersection(line.x, line.y): # ถ้าจุดปลายของ “ส่วนของเส้นตรง” ซักข้างอยู่ในวงกลม ให้คืนค่า True
            return True
        ret = Circle.line_intersection(self, line) # หาค าตอบว่า “เส้นตรง” ตัดกับวงกลมกี่จุด
...
for i in range(n):
    ...
    if Circle.line_in_circle(line, circle): # ”ส่วนของเส้นตรง” ตัดหรือสัมผัสวงกลม
        output.append(circle[0]) # เติมหมายเลขวงกลมลงใน output
if len(output) == 0:
    print('Not Found')
else:
